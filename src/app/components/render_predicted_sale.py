...
def render_predicted_sale(forecast_date=date.today(), forecast_sale=12, store_type="A"):
    ...
    return st.markdown(
        f"""<div style='background-color: {color}; padding: .5rem 1rem; border-radius: .5rem; color: white; margin-bottom: .5rem; display: flex; align-items: center; justify-content: center; flex-direction: column'>
                    <p style='margin-bottom: .1rem'>Date: {forecast_date} | Store Type: {store_type} </p>
                    <h1 style='margin-bottom: -.6rem; margin-top: -.6rem; color: white'>Sales: ${forecast_sale}</h1>
                </div>
            """,
